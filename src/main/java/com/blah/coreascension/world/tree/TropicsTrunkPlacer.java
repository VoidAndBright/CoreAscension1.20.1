...
public class TropicsTrunkPlacer extends TrunkPlacer
{
    ...
    public List<FoliagePlacer.TreeNode> generate(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer,
        ...
        for(int i = 0; i < height_; i++) {
            ...
            if (i == height_ - 1)
            {
                getAndSetState(world, replacer, random, startPos.west(1), config);
                getAndSetState(world, replacer, random, startPos.north(1), config);
                getAndSetState(world, replacer, random, startPos.east(1), config);
                getAndSetState(world, replacer, random, startPos.south(1), config);
            }
